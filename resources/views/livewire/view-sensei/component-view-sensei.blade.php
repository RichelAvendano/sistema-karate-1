        ...
        <div class="panel-custom animated fadeIn" style=' margin: 20px 0;padding:10px'>
            ...
            <div class="modern-container-header" style="justify-content:space-around; flex-wrap:wrap; gap:15px">
                <div class="modern-header" style="margin-bottom:5px;">
                    <span class="text-center color-title" style="font-size:25px">{{ __('Buscar Senseis') }}</span>
                    <span class="text-center color-paragraph" style="margin:0px;">{{ __('aqui se puede buscar, editar y eliminar senseis') }}</span>
                </div>
            </div>
            <!-- Búsqueda -->
            <div class="table-controls">
                <div class="search-box">
                    ...
                    <input wire:model.live="search" type="text" placeholder="Buscar Senseis..." class="search-input">
                </div>
            ...
            <div class="table-container">
                ...
                <table class="glass-table animated fadeIn @if ($changeTable) animated fadeIn @endif" style="min-width: 625px">
                    <thead>
                        <tr>
                            ...
                            <th wire:click="sortByModel('senseis.date_of_birth')" class="sortable">
                                <span>Edad</span>
                                <span>
                                    @if ($sortBy === 'senseis.date_of_birth')
                                        <i class="fa-solid fa-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>
                                    @endif
                                </span>
                            </th>
                            <th wire:click="sortByModel('senseis.dan')" class="sortable">
                                <span>Dan</span>
                                <span>
                                    @if ($sortBy === 'senseis.dan')
                                        <i class="fa-solid fa-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>
                                    @endif
                                </span>
                            </th>
                            <th wire:click="sortByModel('senseis.status')" class="sortable">
                                <span>Estatus</span>
                                <span>
                                    @if ($sortBy === 'senseis.status')
                                        <i class="fa-solid fa-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}"></i>
                                    @endif
                                </span>
                            </th>
                    ...
                    <tbody>
                        @foreach ($senseis as $sensei)
                            <tr wire:key="user-{{ $sensei->id }}"
                                class="{{ $loop->odd ? 'odd-row' : 'even-row' }} hoverable-row">
                                <td style="display: flex; justify-content:center;">
                                    @if ($sensei->photo)
                                        <div class="dojo-photo">
                                            <img src="{{ asset('storage/' . $sensei->photo) }}" alt="{{ $sensei->senseiName }}"
                                                class="dojo-image">
                                        </div>
                                    @else
                                        <div class="dojo-photo">
                                ...
                                <td>{{ $sensei->senseiName }}</td>
                                <td>{{ $sensei->email }}</td>
                                <td>{{ \Carbon\Carbon::parse($sensei->dateOfBirth)->age }} años</td>
                                <td>{{ $sensei->dan }}</td>
                                <td>
                                    @if($sensei->status === 'activo')
                                        <div class="text-center alert-error" style="max-width:70px; background: linear-gradient(135deg, #20c53d, #13702d);padding: 2px 5px;">{{$sensei->status}}</div>
                                    @else
                                        <div class="text-center alert-error" style="max-width:70px;padding: 2px 5px;">{{ $sensei->status }}</div>
                                    @endif
                                    
                                </td>
                                <td>{{$sensei->dojoName}}</td>
                                <td>
                                    <div class="dojo-actions-ultimate">
                                        <button wire:click='studentsSensei({{$sensei->id}},"{{$sensei->status}}")' @click="openStudent = !openStudent" class="new-btn-ultimate new-btn-ultimate-table">
                                            <i class="fa-solid fa-user-graduate"></i> Atletas
                                        </button>
                                        <button wire:click='editModal({{ $sensei->id }})' @click="openEdit = !openEdit" class="edit-btn-ultimate edit-btn-ultimate-table">
                                            <i class="fa-solid fa-pen"></i> Editar
                                        </button>
                                        <button wire:click='destroyModal({{ $sensei->id }},{{$sensei->user_id}})' @click="openDestroy = !openDestroy" class="delete-btn-ultimate delete-btn-ultimate-table">
            ...
            <div class="modal-container " :class="openEdit ? 'animated fadeInDown' : 'animated zoomOut'">
                ...
                <div class="modern-container-header">
                    <div class="modern-header">
                        <span class="text-center color-title">{{ __('Editar Sensei') }}</span>
                        <span class="text-center color-paragraph"
                            style="margin:0px;">{{ __('Edita los Senseis existentes') }}</span>
                    </div>
                </div>
                                    ...
                                    <div class="modern-file-input-container"> {{-- Contenedor para el input de archivo --}}
                                        ...
                                        @if ($photoSensei)
                                            <div class="close-btn-image" 
                                                wire:click.prevent="removephotoSensei"><i class="fa-solid fa-trash"
                        ...
                        <div class="modern-header">
                            ...
                            <div class="" style="display: flex;gap: 10px">
                                <button class="edit-btn-ultimate" 
                                    ...
                                    <span style="font-weight:700">Del Sensei</span>
                                </button>
                                ...
                                <button class="edit-btn-ultimate" 
                                    ...
                                    <span style="font-weight:700">Sin sensei</span>
                                </button>
                            </div>
                        </div>
                    @endif
                        ...
                        <div style="width:100%">
                            <div class="modal-container" style="max-width:500px; margin:0 auto">
                                ...
                                <div class="panel-body">
                                    <h3 class=" text-center alert alert-danger" style="margin:15px 0;font-size:20px">Este Sensei no tiene Alumnos Asignados</h3>
                                </div>
                            </div>
                        </div>
                    @endif
